...
class Role(Base):
    __tablename__ = "role"
    __table_args__: dict[str, Any] = {"schema": "auth"}
...
class User(Base):
    ...
    company_id:           Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("auth.company.id", ondelete="SET NULL"), nullable=True, index=True)
...
class RevokeToken(Base):
    ...
    parent_jti: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
